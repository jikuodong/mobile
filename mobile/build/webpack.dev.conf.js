'use strict'
// 首先引入的是一些工具方法，下面我们就需要去util文件中看一下有哪些对应的工具方法
const utils = require('./utils')
// 引入webpack模块
const webpack = require('webpack')
// 引入配置文件
// 这个配置文件中包含可一些dev和production环境的基本配置
const config = require('../config')
// 引入webpack-merge模块。这个模块用于把多个webpack配置合并成一个配置，后面的配置会覆盖前面的位置
const merge = require('webpack-merge')
const path = require('path')
// 引入webpack的基本配置，这个设置文件包含了开发环境和生产环境的一些公共配置
...
const devWebpackConfig = merge(baseWebpackConfig, {
  ...
  // 文件中进行自定义配置
  devServer: {
    // 用于配置在开发工具的控制台中显示日志级别
    //  注意这个不是对bundle的错误和警告的配置，而是对它生成之前的消息的配置
    clientLogLevel: 'warning',
    // 表示当前使用html5的history api的时候，任意的404响应都需要被替代为index.html
    historyApiFallback: {
      rewrites: [
    ...
    // 一切服务都需要使用gzip压缩
    // 可以在js,css 等文件的response header中发现有Content-Encoding:gzip响应头
    compress: true,
    ...
    // 当编译出现错误的时候，是否希望在浏览器中展示一个全屏蒙层来展示错误信息
    ...
    // 与监视文件相关的控制选项。
    watchOptions: {
      // 如果这个选项为true,会以轮循的方式检查我们的文件变动，效率不好
  ...
  plugins: [
    ...
    // 启用热替换模块
    // 记住，我们永远不要在生产环境中使用hmr
    new webpack.HotModuleReplacementPlugin(),
    // 这个插件的主要作用就是在加载的时候直接返回更新文件的名称，而不是文件的id
    new webpack.NamedModulesPlugin(), // HMR shows correct file names in console on update.
    // 使用这个插件可以在编译出错的时候跳过输出阶段，这样可以确保输出资源不会包含错误。
...
module.exports = new Promise((resolve, reject) => {
  ...
  portfinder.getPort((err, port) => {
    if (err) {
      ...
      // 将FriendlyErrorsPlugin 添加到webpack的配置文件中
