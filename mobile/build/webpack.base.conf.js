...
// 下面就是webpack基本的配置信息（可以是开发环境和生产环境公共的配置）
const vuxLoader = require('vux-loader')
const webpackConfig = {
  // webpack 解析文件时候的根目录（如果把webpack.config.js）放在了项目的根目录下面，这个配置可以省略
  ...
  module: {
    // rules 是一个数组，其中的没一个元素都是一个对象，这个对象是针对具体类型的文件进行配置
    rules: [
      ...(config.dev.useEslint ? [createLintingRule()] : []),
      // .vue文件的配置
      {
        // 这个属性是一个正则表达式，用于匹配wenjian。这里匹配的是.vue文件
        test: /\.vue$/,
        // 指定该种类型文件的加载器名称
        loader: 'vue-loader',
        // 针对此加载器的具体配置
        // 针对前面的分析，这个配置对象中包含了各种css类型文件的配置，css source map 的配置 以及一些transform的配置
  ...
  // 这些选项用于配置polyfill或mock 某些node.js全局变量和模块
  // 这可以使最初为node.js编写的代码可以在浏览器端运行
  node: {
    // 这个配置是一个对象，其中的每个属性都是nodejs全局变量或模块的名称
    // prevent webpack from injecting useless setImmediate polyfill because Vue
    // source contains it (although only uses it if it's native).
    // false 表示什么都不提供。如果获取此对象的代码，可能会因为获取不到此对象而触发ReferenceError错误
    setImmediate: false,
